...
#include "dit_models/components/autoencoders/audio_vae.hpp"
...
namespace LTXV {
    ...
    static ggml_tensor* compute_log_mel_spectrogram(GGMLRunnerContext* runner_ctx,
        ...
        if (left_pad > 0) {
            x = ggml_ext_pad_ext(ctx, x, static_cast<int>(left_pad), 0, 0, 0, 0, 0, 0, 0);
        }
    ...
    struct LTXAudioVAERunner : public AudioVAERunner {
        ...
        LTXAudioVAERunner(ggml_backend_t backend,
            ...
            : AudioVAERunner(backend, offload_params_to_cpu),
        ...
        sd::Tensor<float> decode(int n_threads,
            ...
            auto result = restore_trailing_singleton_dims(GGMLRunner::compute<float>(get_graph, n_threads, false, false), 4);
