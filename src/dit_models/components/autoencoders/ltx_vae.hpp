...
namespace LTXVAE {
    ...
    class PreciseConv3d : public Conv3d {
    public:
        PreciseConv3d(int64_t in_channels,
        ...
        ggml_tensor* forward(GGMLRunnerContext* ctx, ggml_tensor* x) override {
            ...
            ggml_tensor* im2col = ggml_im2col_3d(ctx->ggml_ctx,
                                                 w,
                                                 x,
                                                 in_channels,
                                                 std::get<2>(stride),
                                                 ...
                                                 std::get<0>(dilation),
                                                 w->type);
            const int64_t output_channels = w->ne[3] / in_channels;
            const int64_t batch = x->ne[3] / in_channels;
            x = ggml_mul_mat(ctx->ggml_ctx,
                             ggml_reshape_2d(ctx->ggml_ctx,
                                             im2col,
                                             im2col->ne[0],
                                             im2col->ne[3] * im2col->ne[2] * im2col->ne[1]),
                             ggml_reshape_2d(ctx->ggml_ctx,
                                             w,
                                             w->ne[0] * w->ne[1] * w->ne[2] * in_channels,
                                             output_channels));
            ggml_mul_mat_set_prec(x, GGML_PREC_F32);

            const int64_t output_depth = im2col->ne[3] / batch;
            ...
            if (b != nullptr) {
                b = ggml_reshape_4d(ctx->ggml_ctx, b, 1, 1, 1, b->ne[0]);
    ...
    public:
        CausalConv3d(int64_t in_channels,
            ...
            if (force_prec_f32) {
                blocks["conv"] = std::make_shared<PreciseConv3d>(
                    in_channels,
                    out_channels,
                    std::tuple<int, int, int>{kernel_size, kernel_size, kernel_size},
                    stride,
                    std::tuple<int, int, int>{0, kernel_size / 2, kernel_size / 2},
                    std::tuple<int, int, int>{dilation, 1, 1},
                    bias);
            } else {
                blocks["conv"] = std::make_shared<Conv3d>(
                    in_channels,
                    out_channels,
                    std::tuple<int, int, int>{kernel_size, kernel_size, kernel_size},
                    stride,
...
struct LTXVideoVAE : public VAE {
    ...
    void set_temporal_tiling_enabled(bool enabled) {
        temporal_tiling_enabled = enabled;
    }

    void set_tiling_params(const ed_tiling_params_t& params) {
        temporal_tiling_enabled = false;
    ...
    sd::Tensor<float> decode_temporal_tiled_streaming(const int n_threads,
        ...
        for (int64_t start = 0; start < total_frames - plan.overlap; start += plan.stride) {
            ...
            auto chunk = restore_trailing_singleton_dims(GGMLRunner::compute<float>(get_graph, n_threads, true, false),
                                                         expected_dim);
            if (chunk.empty()) {
                free_cache_ctx_and_buffer();
                cache_tensor_map.clear();
    ...
    sd::Tensor<float> _compute(const int n_threads,
        ...
        auto result = restore_trailing_singleton_dims(GGMLRunner::compute<float>(get_graph, n_threads, false, false), expected_dim);
        if (result.empty()) {
            return {};
        }
        return result;
    ...
    sd::Tensor<float> apply_latent_statistics(const int n_threads,
        ...
        return restore_trailing_singleton_dims(GGMLRunner::compute<float>(get_graph, n_threads, false, false),
