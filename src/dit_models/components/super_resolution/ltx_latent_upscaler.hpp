...
#include "core/backend/ggml/ggml_extend.hpp"
#include "core/backend/ggml/ggml_graph_cut.h"
...
namespace LTXVUpsampler {
    ...
    public:
        ...
        ggml_tensor* forward(GGMLRunnerContext* ctx, ggml_tensor* x) {
            ...
            GGML_ASSERT(ctx->bind_backend_tensor_data);
    ...
    struct LatentUpsamplerRunner : public GGMLRunner {
        ...
        sd::Tensor<float> compute(const int n_threads,
            ...
            return restore_trailing_singleton_dims(
                GGMLRunner::compute<float>(get_graph, n_threads, false, false),
                expected_dim);
        }
    };
