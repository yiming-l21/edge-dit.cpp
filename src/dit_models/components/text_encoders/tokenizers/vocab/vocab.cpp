...
std::string load_gemma_merges() {
    return std::string(reinterpret_cast<const char*>(gemma_merges_utf8_c_str),
                       sizeof(gemma_merges_utf8_c_str));
}

std::string load_gemma_vocab_json() {
    return std::string(reinterpret_cast<const char*>(gemma_vocab_json_utf8_c_str),
                       sizeof(gemma_vocab_json_utf8_c_str));
}
