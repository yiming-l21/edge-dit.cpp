    ...
    for (auto& splited_text : splited_texts) {
        ...
        for (auto& token : tokens) {
            ...
            for (auto bpe_str : bpe_strs) {
                ...
                if (iter == encoder.end() && byte_fallback) {
                    const std::string bytes = utf32_to_utf8(bpe_str);
                    for (unsigned char b : bytes) {
                        char hex_buf[16];
                        snprintf(hex_buf, sizeof(hex_buf), "<0x%02X>", b);
                        auto fallback = encoder.find(utf8_to_utf32(hex_buf));
                        bpe_tokens.push_back(fallback != encoder.end() ? fallback->second : UNK_TOKEN_ID);
                        token_strs.push_back(hex_buf);
                    }
                    continue;
                }
                bpe_tokens.push_back(iter != encoder.end() ? iter->second : UNK_TOKEN_ID);
